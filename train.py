...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    #   中心点(m,n,2) 宽高(m,n,2)
    # -----------------------------------------------------------#
    boxes_xy = (true_boxes[..., 0:2] + true_boxes[..., 2:4]) // 2  # 除后为整数 中心点坐标
    boxes_wh = true_boxes[..., 2:4] - true_boxes[..., 0:2]  # 真实框的大小
    # -----------------------------------------------------------#
    #   将真实框归一化到小数形式
    # -----------------------------------------------------------#
...
if __name__ == "__main__":
    ...
    checkpoint = ModelCheckpoint(log_dir + 'ep{epoch:03d}-loss{loss:.3f}-val_loss{val_loss:.3f}.h5',
                                 monitor='val_loss', save_weights_only=False, save_best_only=True, period=30)
    early_stopping = EarlyStopping(monitor='val_loss', min_delta=0, patience=8000, verbose=1)
    ...
    if flag_warmUp:
        ...
        if Cosine_scheduler:
            ...
            AdamOptimizer = Adam(learning_rate_base)
            reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.98, patience=10, verbose=1)
            # reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.98, patience=10, verbose=1)
        ...
        model.fit(data_generator(lines[:num_train], batch_size, input_shape, anchors, num_classes, mosaic=mosaic),
                  ...
                  validation_data=data_generator(lines[num_train:], batch_size_eval, input_shape, anchors, num_classes,
                                                 mosaic=False),
                  validation_steps=max(1, num_val // batch_size_eval),
                  epochs=Freeze_epoch,
                  initial_epoch=Init_epoch,
                  callbacks=[checkpoint, reduce_lr, early_stopping, printLR])
        # model.save_weights(log_dir + 'trained_weights_stage_1.h5')
        # model.save(log_dir + 'yolov4TinyStrawberry')
    ...
    if True:
        ...
        model.fit(data_generator(lines[:num_train], batch_size, input_shape, anchors, num_classes, mosaic=mosaic),
                  ...
                  callbacks=[checkpoint, reduce_lr, early_stopping, printLR])
        model.save(log_dir + 'yolov4TinyStrawberry')
        model.save_weights(log_dir + 'last_pruneYolov4TinyStrawberry.h5')
