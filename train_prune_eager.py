...
if __name__ == "__main__":
    ...
    if True:
        ...
        if Use_Data_Loader:
            ...
            gen_val = partial(data_generator, annotation_lines=lines[num_train:], batch_size=batch_size_eval,
            ...
            # gen = gen.cache().shuffle(batch_size).prefetch(buffer_size=AUTOTUNE)
            # gen_val = gen_val.cache().prefetch(buffer_size=AUTOTUNE)
            gen = gen.shuffle(batch_size).prefetch(buffer_size=AUTOTUNE).cache()
            gen_val = gen_val.prefetch(buffer_size=AUTOTUNE).cache()
        else:
            gen = data_generator(lines[:num_train], batch_size, input_shape, anchors, num_classes, mosaic=mosaic)
            gen_val = data_generator(lines[num_train:], batch_size_eval, input_shape, anchors, num_classes, mosaic=False)
